...
u32 LoadBootImage(void)
{
	...
	if ((Silicon_Version != SILICON_VERSION_1) &&
		...
		/* Allow testing of RSA without burning eFUSEs, this is not a secure system
		 */
		...
		*/
		if (PartitionHeader[0].ACOffset != 0) {
			/* The BootROM normally copies the PPK AC into OCM but only does it when the RSA enable
			* efuse is programmed. There is a special linker section that already exists but is
			* not normally used so copy the PPK AC into it rather than just at the end of the
			* FSBL text section which seems to cause problems of overwriting the data section.
			*/
			fsbl_printf(DEBUG_GENERAL, "*** RSA Without eFUSE, Not a secure system! ***\n\r");

			/* Force RSA regardless of the eFUSE so no eFUSE programming is required for early testing
			*/
			...
			/* Keep using the FSBL length variable but use the address of the special section for
...
#ifndef NO_CPU_WARM_REBOOT
			/* For a warm CPU only restart leave the PL along not loading it.
			 * The CPU WDT is used to do a warm reboot of the CPU so check
			 * to see if the WDT caused the reset.
			 */
			if ((Xil_In32(0xF800702c) & 0x80000000) != 0) {
				xil_printf("WDT reset, skipping PL\r\n");
				PartitionNum++;
				BitstreamFlag = 0;
				continue;
			}
...
#define QSPI_FLASH_HIGHADDR			XPAR_PS7_QSPI_LINEAR_0_S_AXI_HIGHADDR
		...
		/* For no DDR systems, with XIP from QSPI and RSA, the signatures
		 * need to be verified in place in QSPI and then SSBL must be used
		 * to copy the primary app data from QSPI to high OCM after FSBL
		 * is no longer needed in OCM.
		 */
		 ...
		 */
		if ((PSPartitionFlag == 1) && ((PartitionLoadAddr >= 0xFFFC0000) && (PartitionLoadAddr < 0xFFFF0000))) {
